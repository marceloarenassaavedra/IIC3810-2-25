...
\bf Tarea 1\\
\bf Fecha de entrega: miércoles 3 de septiembre
\end{center}

\vspace{1cm}

\begin{enumerate}
\item Demuestre que para todo lenguaje $L$, se tiene que $L \in \np \cap \co\np$ si y sólo si $\np^L = \np$.
  
\item Sea $\text{P}^{\np[1]}$ definido como $\text{P}^{\np}$ pero solo permitiendo una llamada al oráculo para un lenguaje en $\np$. Demuestre que si $\text{P}^{\np[1]} \subseteq \np \cup \co\np$, entonces $\np = \co\np$.

\item Demuestre que el problema $\br$ visto en clases es $\Pi_2^p$-hard.
\end{enumerate}
